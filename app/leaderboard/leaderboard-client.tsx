...
import { LeaderboardMetric, Timeframe, LeaderboardEntry } from "@/lib/leaderboard/types";
...
export default function LeaderboardClient({
  ...
  const apiUrl = `/api/leaderboards/users?${queryParams}`;
  ...
  // Find current user's entry and stats from the leaderboard data
  const currentUserEntry = useMemo(() => {
    const index = entries.findIndex((e: LeaderboardEntry) => e.userId === currentUserId);
    if (index === -1) return null;
    return {
      entry: entries[index] as LeaderboardEntry,
      rank: index + 1,
    };
  ...
  const bannerData = currentUserEntry
    ? {
        // User IS ranked - use real leaderboard data
        avatar: currentUserEntry.entry.image || userInfo?.image || null,
        title: currentUserEntry.entry.username || userInfo?.username || "Anonymous",
        rank: currentUserEntry.rank,
        pnl: currentUserEntry.entry.realizedPnl,
        winRate: currentUserEntry.entry.winRate,
        trades: currentUserEntry.entry.totalTrades,
        volume: currentUserEntry.entry.volume,
      }
    : userInfo
    ? {
        // User exists but NOT ranked yet - show identity with zeros
  ...
  return (
    <div className="space-y-6">
      {/* ═══════════════════════════════════════════════════════════════════════
          HERO BANNER - Shows current user's real stats from leaderboard data
      ═══════════════════════════════════════════════════════════════════════ */}
      {bannerData ? (
        <LeaderboardHero
          type="user"
          ...
          crewBadge={userInfo?.crew || null}
          isLoading={isLoading && !data}
        />
      ) : (
        <LeaderboardHeroEmpty type="user" />
      )}
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-white/40">
              ...
              <span className="text-white font-medium">{total}</span> traders
            </p>
          </div>

          {/* Leaderboard Table */}
          <LeaderboardTable entries={entries} metric={metric} currentUserId={currentUserId} />
        </>
      )}
    </div>
  );
