...
import SeasonLeaderboardTable from "./season-leaderboard-table";
import { LeaderboardMetric, LeaderboardEntry } from "@/lib/leaderboard/types";
...
export default function SeasonLeaderboardClient({
  ...
  const apiUrl = `/api/leaderboards/seasons/${season.id}/users?${queryParams}`;
    ...
    <div className="space-y-6">
      ...
      {data && !error && (
        <>
          ...
          {/* Leaderboard Table */}
          <SeasonLeaderboardTable entries={entries} metric={metric} />
        </>
      )}
    </div>
  );
