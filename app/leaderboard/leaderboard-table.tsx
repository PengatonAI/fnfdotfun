...
import { LeaderboardEntry, LeaderboardMetric } from "@/lib/leaderboard/types";
        ...
        <div className="relative flex flex-col items-center gap-6 text-center">
          ...
          <div>
            ...
            <p className="text-white/40 max-w-md">
              Rankings will appear here as more traders join the competition. Start trading to be the first on the leaderboard!
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="relative rounded-xl bg-gradient-to-br from-[#0d0d0d] via-[#111111] to-[#0a0a0a] border border-[#1a1a1a] shadow-[0_4px_20px_rgba(0,0,0,0.3)] overflow-hidden">
      ...
      <div className="divide-y divide-[#1a1a1a]">
        {entries.map((entry, index) => {
          ...
          const displayHandle = entry.xHandle ? `@${entry.xHandle}` : null;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="relative flex-shrink-0">
                      <div className={`relative h-10 w-10 rounded-full overflow-hidden bg-[#1a1a1a] flex items-center justify-center border ${
                        isCurrentUser ? "border-accent/50" : "border-[#2a2a2a]"
                      }`}>
                        {entry.image ? (
                          <Image
                            src={entry.image}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`font-medium truncate ${isCurrentUser ? "text-white" : "text-white/90"}`}>
                          {displayName}
                        </span>
                        {isCurrentUser && (
